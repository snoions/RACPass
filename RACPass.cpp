 ...
 */
static Function * checkRACPassInterfaceFunction(Value *FuncOrBitcast) {
	...
	Stream << "RACPass interface function redefined: " << *FuncOrBitcast;
	report_fatal_error(StringRef(Err));
} 

namespace {

	struct RACPass : public FunctionPass {
		RACPass() : FunctionPass(ID) {}
	...
	private:
		...
		Function * RACVolatileStore[kNumberOfAccessSizes];
#ifdef ENABLEATOMIC
		Function * RACAtomicInit[kNumberOfAccessSizes];
		Function * RACAtomicLoad[kNumberOfAccessSizes];
		Function * RACAtomicStore[kNumberOfAccessSizes];
		Function * RACAtomicRMW[AtomicRMWInst::LAST_BINOP + 1][kNumberOfAccessSizes];
		Function * RACAtomicCAS_V1[kNumberOfAccessSizes];
		Function * RACAtomicCAS_V2[kNumberOfAccessSizes];
		Function * RACAtomicThreadFence;
#endif
...
StringRef RACPass::getPassName() const {
	return "RACPass";
}


NVMOP RACPass::whichNVMoperation(Instruction *I){
...
void RACPass::initializeCallbacks(Module &M) {
	...
	for (unsigned i = 0; i < kNumberOfAccessSizes; i++) {
...
#ifdef ENABLEATOMIC
		SmallString<32> VolatileLoadName("rac_volatile_load" + BitSizeStr);
		SmallString<32> VolatileStoreName("rac_volatile_store" + BitSizeStr);
		SmallString<32> AtomicInitName("rac_atomic_init" + BitSizeStr);
		SmallString<32> AtomicLoadName("rac_atomic_load" + BitSizeStr);
		...
		SmallString<32> AtomicCASName_V1("rac_atomic_compare_exchange" + BitSizeStr + "_v1");
...
bool RACPass::addrPointsToConstantData(Value *Addr) {
...
#ifdef ENABLEATOMIC
bool RACPass::shouldInstrumentBeforeAtomics(Instruction * Inst) {
...
#endif
void RACPass::chooseInstructionsToInstrument(
...
bool RACPass::instrumentLoadOrStore(Instruction *I, const DataLayout &DL) {
	IRBuilder<> IRB(I);
	...
	Function *OnAccessFunc = IsWrite ? RACStore[idx] : RACLoad[idx];
...
#ifdef ENABLEATOMIC
bool RACPass::instrumentVolatile(Instruction * I, const DataLayout &DL) {
	...
	if (LoadInst *LI = dyn_cast<LoadInst>(I)) {
		...
		Value *args[] = {IRB.CreatePointerCast(addr, ptrTy), position};
        Type *orgTy = val->getType();
		Value *funcInst = IRB.CreateCall(RACVolatileLoad[idx], args);
		Value *cast = IRB.CreateBitOrPointerCast(funcInst, orgTy);
		I->replaceAllUsesWith(cast);
	} else if (StoreInst *SI = dyn_cast<StoreInst>(I)) {
		assert( SI->isVolatile() );
		//assert(RACVolatileStore[idx]->arg_size()> 2);
		//Type *valType = RACVolatileStore[idx]->getArg(1)->getType();
		//if( val->getType() != valType) {
                //        errs() << "RAC Warning: Value Type= " << *val->getType() << "\t casting to ==>" << *valType << '\n';
		//	val = CastInst::CreatePointerCast(val, valType);
		//}
		Value *args[] = {IRB.CreatePointerCast(addr, ptrTy),
				IRB.CreateBitOrPointerCast(val, Ty), position};
		CallInst *C = CallInst::Create(RACVolatileStore[idx], args);
		ReplaceInstWithInst(I, C);
	} else {
		return false;
	}
...
bool RACPass::instrumentMemIntrinsic(Instruction *I) {
...
#ifdef ENABLEATOMIC
bool RACPass::instrumentAtomic(Instruction * I, const DataLayout &DL) {
	...
	if (LoadInst *LI = dyn_cast<LoadInst>(I)) {
		...
		Instruction* funcInst = CallInst::Create(RACAtomicLoad[Idx], args);
		...
		Instruction* funcInst = CallInst::Create(RACAtomicStore[Idx], args);
		...
		Value *order = ConstantInt::get(OrdTy, atomic_order_index);
		Value *args[] = {Addr, val, order, position};
		Instruction* funcInst = CallInst::Create(RACAtomicRMW[RMWI->getOperation()][Idx], args);
		...
		CallInst *funcInst = IRB.CreateCall(RACAtomicCAS_V1[Idx], Args);
		...
		CallInst *funcInst = CallInst::Create(RACAtomicThreadFence, Args);
...
bool RACPass::isAtomicCall(Instruction *I) {
	if ( auto *CI = dyn_cast<CallInst>(I) ) {
...
bool RACPass::instrumentAtomicCall(CallInst *CI, const DataLayout &DL) {
	...
	if (funName.contains("atomic_init")) {
		...
		Instruction* funcInst = CallInst::Create(RACAtomicInit[Idx], args);
	...
	if (funName.contains("atomic_load")) {
		...
		CallInst *funcInst = IRB.CreateCall(RACAtomicLoad[Idx], args);
	...
	if (funName.contains("atomic_fetch_") || 
		...
		Instruction* funcInst = CallInst::Create(RACAtomicRMW[op][Idx], args);
		ReplaceInstWithInst(CI, funcInst);
	...
	*/
	if ( funName.contains("atomic_compare_exchange_") ) {
		...
		Instruction* funcInst = CallInst::Create(RACAtomicCAS_V2[Idx], args);
		ReplaceInstWithInst(CI, funcInst);
...
#endif
int RACPass::getMemoryAccessFuncIndex(Value *Val,
										const DataLayout &DL) {
...
PreservedAnalyses RACWrapperPass::run(Function &F, FunctionAnalysisManager &AM) {
	RACPass Pass;
...
static RegisterPass<RACPass> X("racoherence", "Instrumentation Pass for RACoherence protocol",
                                 false /* Only looks at CFG */,
                                 true/* Transformation Pass */);

PassPluginLibraryInfo getPassPluginInfo() {
	return {LLVM_PLUGIN_API_VERSION, "RAC", LLVM_VERSION_STRING,
          [](PassBuilder &PB) {
			PB.registerPipelineEarlySimplificationEPCallback(
				[](ModulePassManager &MPM, OptimizationLevel Level, auto) {
					MPM.addPass(createModuleToFunctionPassAdaptor(RACWrapperPass()));
				});
          }};
}
